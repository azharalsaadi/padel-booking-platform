...
const JOURNEY_STEPS = [
  { step: 1, title: 'Select Date', description: 'Choose your preferred date and available time slot.' },
  { step: 2, title: 'Review', description: 'Review your selected session and booking total.' },
  { step: 3, title: 'Play', description: 'Complete your booking and arrive ready to play.' },
]

const OFFER_COPY: Record<number, { label: string; description: string; featured?: boolean }> = {
  1: { label: '1 Hour', description: 'Best for a quick game' },
  2: { label: '2 Hours', description: 'Perfect for extended sessions', featured: true },
  3: { label: '3 Hours', description: 'Maximum play, best value for groups' },
}
...
export function LandingPage() {
  ...
  return (
    <>
          ...
          <div className="relative">
            ...
            <div className="absolute inset-0 flex items-center">
              <div className="max-w-lg px-6 py-8 sm:px-12 sm:py-10 lg:px-16">
                <p className="text-xs font-semibold tracking-[0.2em] text-white/70 uppercase">Oman&apos;s Premium Padel Destination</p>
                ...
                <h1 className="font-serif text-3xl leading-tight font-semibold text-white sm:text-4xl lg:text-5xl">
                  Book Premium Padel Courts <br />
                  in Seconds
                </h1>
                <p className="mt-4 max-w-md text-sm text-white/85 sm:text-base">
                  Reserve indoor and outdoor premium padel courts in just a few clicks. Experience luxury facilities,
                  seamless booking, and world-class courts.
                </p>
                <Link
                  to="/book"
                  className={cn(
      ...
      {/* Offers */}
      <section id="offers" className="scroll-mt-20 bg-background py-20 sm:py-28">
        <Container className="flex flex-col gap-10">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              <h2 className="font-serif text-3xl font-semibold text-text">Play More, Pay Less</h2>
              <p className="mt-2 text-sm text-text-muted">Exclusive rates for extended sessions at Oman&apos;s premier club.</p>
            </div>
            <a
              href="#offers"
            ...
            {offerQueries.map((query, index) => {
              ...
              return (
                ...
                >
                  ...
                  {query.isError && <p className="text-sm text-danger">Could not load this rate.</p>}

                  {quote && (
                    <>
                      <p className="font-serif text-3xl font-bold text-text">{formatWholeOmr(quote.applied_rule.price_per_hour_baisa)}</p>
                      <p className="text-xs text-text-muted">per hour</p>
      ...
      {/* Journey */}
      <section id="how-to-book" className="scroll-mt-20 bg-secondary-50 py-20 sm:py-28">
        <Container className="flex flex-col gap-10">
          <div>
            <h2 className="font-serif text-3xl font-semibold text-text">Your Journey to the Court</h2>
            <p className="mt-2 max-w-md text-sm text-text-muted">
              A seamless experience from the moment you decide to play until you step onto the turf.
            </p>
          </div>
        ...
        <Container>
          <div className="relative isolate overflow-hidden rounded-card">
            ...
            <div className="relative flex flex-col items-center gap-4 px-6 py-16 text-center sm:py-20">
              <h2 className="font-serif text-3xl font-semibold text-white sm:text-4xl">Ready to Play?</h2>
              <p className="max-w-md text-white/85">
                Your court is waiting. Join the elite community of Rally and experience padel like never before.
              </p>
              <Link
                to="/book"
                className={cn(
