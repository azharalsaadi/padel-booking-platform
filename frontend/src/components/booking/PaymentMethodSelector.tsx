...
export function PaymentMethodSelector({ value, onChange, error }: PaymentMethodSelectorProps) {
  ...
  const OPTIONS: Array<{
    ...
    {
      value: 'thawani',
      label: t('booking.payOnlineWithThawani'),
      description: t('booking.payOnlineDescription'),
      icon: CreditCard,
      badges: [
        { label: t('booking.recommended'), variant: 'success' },
        { label: t('booking.secure'), variant: 'neutral' },
      ],
    },
    {
      value: 'pay_at_venue',
      label: t('booking.payAtVenue'),
      description: t('booking.payAtVenueDescription'),
      icon: MapPin,
    },
  ]
  ...
  return (
    <fieldset className="flex flex-col gap-4" aria-describedby={error ? errorId : undefined}>
      <legend className="sr-only">{t('booking.paymentMethod')}</legend>
      <div aria-hidden="true" className="flex items-center gap-3">
        ...
        <h2 className="font-serif text-xl font-semibold text-text">{t('booking.paymentMethod')}</h2>
      </div>
